...
class SynapseBaseDataset(Dataset):
...
class Synapse_dataset(SynapseBaseDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)


class SynapseDatasetFast(SynapseBaseDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.split == "train":
    ...
    def __load_all_data(self):
        ...
        # pbar = tqdm(self.sample_list, desc="loading all slices")
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            image, label = self.images[idx].copy(), self.labels[idx].copy()
            image, label = augment_seg(self.img_aug, image, label)
            x, y = image.shape
            if x != self.img_size or y != self.img_size:
                image = zoom(image, (self.img_size / x, self.img_size / y), order=3)  # why not 3?
